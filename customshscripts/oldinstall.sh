...
if [[ ! "$is_arch" =~ ^[yY]$ ]]; then
    ...
    echo -e "   ${GRAY}──────────────────────────────────────${RESET}"
    echo -e "   1. Copy configuration folders to ${BLUE}~/.config/${RESET}:"
    echo -e "      ${GRAY}hypr, kitty, mpd, rmpc, swaync, vicinae, waybar, fish${RESET}"
    echo -e "   2. Setup the scripts directory:"
    echo -e "      ${BLUE}mkdir -p ~/myscripts${RESET}"
    echo -e "   3. Copy and enable the theme switcher:"
    echo -e "      ${GRAY}cp \"customshscripts/personal scripts/theme_switcher.sh\" ~/myscripts/${RESET}"
    echo -e "      ${GRAY}chmod +x ~/myscripts/theme_switcher.sh${RESET}"
    echo -e "   4. Install dependencies manually (see README.md)."
    echo ""
    exit 0
fi
...
# --- STEP 6.1: SCRIPT DEPLOYMENT ---
# Ensure the destination directory exists
run_cmd "Creating myscripts directory" "mkdir -p $HOME/myscripts"

# Copy theme_switcher.sh with proper quoting for spaces
if [ -f "$SCRIPT_DIR/customshscripts/personal scripts/theme_switcher.sh" ]; then
    run_cmd "Installing theme_switcher.sh script" "cp \"$SCRIPT_DIR/customshscripts/personal scripts/theme_switcher.sh\" \"$HOME/myscripts/\" && chmod +x \"$HOME/myscripts/theme_switcher.sh\""
fi
