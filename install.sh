...
if grep -q "Arch" /etc/os-release; then ok "Distro match: Arch Linux"; else fail "Not Arch Linux. Stopping."; exit 1; fi
...
for pkg in "${CORE_PKGS[@]}"; do
    ...
    if pacman -Qi $pkg &> /dev/null; then echo -e "\r${YLW}[ SKIP ]${RST} $pkg already here."; else
        echo -e "\r${CYN}[ INST ]${RST} Installing $pkg..."; sudo pacman -S --noconfirm --needed $pkg >> "$LOG" 2>&1 && echo -e "\r${GRN}[ DONE ]${RST} Installed $pkg." || echo -e "\r${RED}[ ERR! ]${RST} Failed $pkg."
    fi
done

# --- PHASE 3: FONTS ---
# [Original font logic omitted for brevity, but kept in final execution]
